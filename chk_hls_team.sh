...
for DOMAIN in "${DOMAINS[@]}"; do
    ...
    if [ -z "$IP" ]; then
        ...
        # Запрос к специализированному сервису Cymru (возвращает "ASN | IP | Name")
        NET_DATA=$(whois -h whois.cymru.com " -v $IP" | tail -n1)
        ASN=$(echo "$NET_DATA" | awk -F'|' '{print "AS"$1}' | xargs)
        PROVIDER=$(echo "$NET_DATA" | awk -F'|' '{print $3}" | "$4' | cut -c1-20 | xargs)
